...
public class PlayerShooting : MonoBehaviour {
    ...
    BulletSelector BS;
    AudioSource gunPower;
    public AudioClip gunPowerUp;
    public AudioClip gunOrig;
    ...
    void Shoot() {
        ...
        gunAudio.pitch = Random.Range(0.7f, 1.8f);
        if (bounce) {
            gunAudio.pitch = Random.Range(0.5f, 1.8f);
        }
        if (piercing) {
            gunAudio.pitch = Random.Range(1.0f, 1.8f);
        }
        if (piercing & bounce) {
            gunAudio.pitch = Random.Range(0.9f, 1.9f);
        }

        if (piercing || bounce)
        {
            gunAudio.clip = gunPowerUp;
        }else
        {
            gunAudio.clip = gunOrig;
        }
        gunAudio.Play();
